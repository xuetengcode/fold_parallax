...
def init_output(OUTPUT_PATH, outfile_base, ok_data, play_sound=True, additional=True):
...
def init_log(OUTPUT_PATH, outfile_base, idx, ok_data, additional=True):
    check_dir(OUTPUT_PATH)
    if additional:
        year, month, day, hour, minutes = map(int, time.strftime("%Y %m %d %H %M").split())
        time_str = '-'.join([str(year), expand_data(str(month)), expand_data(str(day)), expand_data(str(hour)), expand_data(str(minutes))])
...
def write2file(csvhdl,data):
    ...
    return
def log2file(data,OUTPUT_PATH, OUTPUT_FILE, ok_data):
    
    print('Saving log to file')
    for i_exp in range(len(data)):
        csvhdl = init_log(OUTPUT_PATH, OUTPUT_FILE, i_exp, ok_data)
